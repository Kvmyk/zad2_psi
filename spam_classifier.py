...
# UWAGA: Zmień ścieżkę do pliku na swoją
df = pd.read_csv('spam.csv', encoding='latin-1')

# Dataset ma dziwną strukturę - usuń puste kolumny
df = df[['v1', 'v2']]
df.columns = ['label', 'message']

print(f"✓ Wczytano {len(df)} wiadomości SMS")
print(f"\nRozkład klas:")
...
print(f"\nStatystyki długości wiadomości:")
...
print(f"\nPrzykład przed i po preprocessingu:")
print(f"ORYGINAŁ: {df['message'].iloc[0]}")
...
print(f"✓ BoW - wymiar: {X_train_bow.shape}")
...
print(f"✓ TF-IDF - wymiar: {X_train_tfidf.shape}")
...
# Testuj z różnymi reprezentacjami
...
# ============================================================================
# KROK 6: EWALUACJA I PORÓWNANIE
# ============================================================================
print("\n" + "=" * 80)
print("[KROK 6] EWALUACJA I PORÓWNANIE WYNIKÓW")
print("=" * 80)
...
# Szczegółowy raport dla najlepszego modelu
print(f"\n📋 SZCZEGÓŁOWY RAPORT - {best_result['Model']} ({best_result['Vectorizer']}):")
...
print(f"False Positives (FP): {cm[0,1]} - Ham błędnie jako Spam")
print(f"False Negatives (FN): {cm[1,0]} - Spam błędnie jako Ham")
...
ax2.set_title('Porównanie F1-Score', fontsize=14, fontweight='bold')
...
# 7.4 Rozkład długości wiadomości
...
ax4.set_title('Rozkład długości wiadomości', fontsize=14, fontweight='bold')
ax4.set_xlabel('Długość znakowa')
ax4.set_ylabel('Liczba wiadomości')
ax4.legend()
ax4.grid(axis='y', alpha=0.3)

# 7.5 Mapa ciepła wszystkich metryk
...
ax5.set_title('Wszystkie metryki - Mapa ciepła', fontsize=14, fontweight='bold')

# 7.6 Porównanie Precision vs Recall
...
print("✓ Wykres zapisany jako 'spam_classification_results.png'")
plt.show()
...
def predict_message(message):
    """Funkcja do predykcji nowej wiadomości"""
    # Preprocessing
...
for i, msg in enumerate(test_messages, 1):
    ...
    print(f"   PREDYKCJA: {result} (pewność: {confidence:.2f}%)")
    print()
